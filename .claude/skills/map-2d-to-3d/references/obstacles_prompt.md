...
All twelve models are built. Chapters 10 and 22-30 have no trees (22-30 confirmed: no tree tile on any of them).
...
机械要塞的内部：黑色的深坑之间铺着铁丝网地板，四周是褐色的岩壁，地上有圆形的通风口和
方形的舱门，红 / 蓝宝箱；左下角岩壁上站着一个机器人（事件触发）。

| Object | DefinitionKey | Footprint (rows × cols) |
|---|---|---|
| 机器人 | — (`_erase`) | painted 2 × 1 (88 over 90), not modelled |

Resolved -- the map is 31 × 47 tiles, all ordinary tiles. **No obstacle models.**
The robot at (2, 45..46) is an event creature the chapter script will place, so
`obstacles_26.json` (`prop_obstacles.py --stack _erase=88/90`) only erases it from
the render map: 88 fills with the rock 50, 90 with 72; `ShapeMatrix` keeps the two
`Gap` tiles.

**The wire mesh stands three voxels above the pits**, the height of chapter 03's
bridge over its water. Both are painted in the same pure black, so
`shapes_to_vox.py --flat --lower 000000=3 --lower-min-area 45` sinks only black
blobs of 45+ pixels: the pits (and the square hatches 37, 196 pixels) go down,
the two-pixel gaps between the wires stay up with the mesh. The vents (36), the
hatches (37), the chests (44 / 46) and the white glow at the exit (84..87) stay
painted tiles. No trees.

## Chapter 27

洞窟里的石头神殿：灰色的石台层层叠起，顶上一方蓝色的水池，池中立着三块石碑；台上
三十多根石柱（高的三格、矮的两格）、六座骑士石像和十二根石球柱，中央一座石头神龛，
两道长石阶，一块浅灰的石板；四周褐色岩壁上立着三十根蓝色的光柱（高的两格、矮的
一格），底部一间小石室里六个宝箱。所有道具下面都画了地面上的倒影。

| Object | DefinitionKey | Footprint (rows × cols) |
|---|---|---|
| 高光柱 ×22 | `light_pillar_1` | **new** -- 1 × 1, painted 2 × 1 (32 over 40, 36 over 41; two 40s on row 1 have their flame off the top) |
| 矮光柱 ×10 | `light_pillar_2` | **new** -- 1 × 1 (42) |
| 高石柱 ×6 | `stone_column_1` | reuse from chapter 21 -- ball 180 over shaft 182 over shaft-and-base 184 |
| 两格石柱 ×16 | `stone_column_3` | **new** -- 180 over 184; stone_column_1 a tile shorter |
| 骑士石像 ×6 | `stone_statue_1` | reuse from chapter 08 -- bust 188 over pedestal 186 |
| 石球柱 ×12 | `stone_pillar_1` | reuse from chapter 09 -- ball 190 over pedestal 186 |
| 石碑 ×3 | `stone_stele_1` | **new** -- cap 224 / 228 over tablet 226 / 230, standing in the pool |
| 石头神龛 | `stone_shrine_1` | reuse from chapter 21 -- roof 160 164 166, tablet 168 170 172 (170 is chapter 21's 154), base 174 175 176, foot 177 178 179 |

Resolved -- the map is 31 × 57 tiles, all ordinary tiles; `obstacles_27.json` was
generated by `prop_obstacles.py` with the shrine merged in from
`obstacles_27_shrine.json`. Every prop stands on its `Blocked` tile: 184 for the
columns (185 under it is the base's reflection, not a base), 186 for the statues
and ball pillars, 40 / 41 for the light pillars, 226 / 230 for the steles.

**The reflections are erased, not modelled.** The art paints a reflection on the
floor under every prop -- pale streaks under the columns and statues (185, 187),
a lit ellipse under the light pillars (on 40 / 41 / 42, and alone as 46), the
blue ghost of the centre stele (227). The user asked for these to be ignored, so
`_erase` entries (`--stack _erase=185,187,227,46`) put those tiles back to the
floor -- 111 on the terrace, 74 on the rock, 75 in the pool -- and no model has
a reflection. The lone ellipses 46 are `Gap` in `ShapeMatrix` and stay so.

Fills are the painted ground: 74 under the light pillars, 111 under everything on
the terraces (forced for 182 / 184 / 185 / 186 / 188, whose art is mostly prop),
206 (the rock bank) under the centre stele's cap, 75 under the steles' bodies.

The long stairs (128..139, `Plain` -- creatures walk them), the terrace edges
(112..127, 144..159), the pale slabs (212..214, 218..223), the rock bumps
(100..105) and the chests (140, 142) stay painted tiles. No trees.
`shapes_to_vox.py --flat --lower 000000=3 --lower 345094=2 --lower-min-area 45`:
the void sinks three voxels, the pool two.

## Chapter 28

水神殿的东翼：灰色的石地上一排蓝色的水池，池边石球柱、石碑和蓝色光柱，右侧一座
石头神龛立在水池上沿；左下一片铁丝网地板，有通风口和舱门；右上角褐色岩壁。

| Object | DefinitionKey | Footprint (rows × cols) |
|---|---|---|
| 高光柱 ×10 | `light_pillar_1` | 196 over 201; the 196 on row 26 has its base off the bottom |
| 矮光柱 ×6 | `light_pillar_2` | 202 |
| 石球柱 ×2 | `stone_pillar_1` | reuse from chapter 09 -- ball 94 over pedestal 90 |
| 石碑 ×4 | `stone_stele_1` | cap 246 / 244 over tablet 233 / 189 |
| 石头神龛 | `stone_shrine_1` | reuse from chapter 21 -- roof 64 66 70, tablet 72 74 76, base 240 241 242 on the pool's top edge; no foot row |

Resolved -- the map is 48 × 26 tiles, all ordinary tiles; `obstacles_28.json` from
`prop_obstacles.py` with `obstacles_28_shrine.json`. Reflections erased as in
chapter 27: 91 under the ball pillars, 234 / 235 and 190 / 191 under the steles,
the lone ellipses 206. Fills forced to the floor 121 for 90 / 190 / 233 / 234
(art mostly prop); the stele at (36, 22) stands in the pool's bottom edge, so its
cap and body fill with water 122 -- the edge line is missing for that one tile.
The mesh floor (124..126), the pale steps under the pools (217..222, 92 / 18 /
93) and the chests stay painted; same `shapes_to_vox.py` flags as chapter 27.

## Chapter 29

三头龙的神殿：顶上三条红 / 灰 / 蓝的龙颈垂下来（画在瓦片上，事件用），下面是浅蓝色
的迷宫地面和褐色砖墙，一道石阶通到一方小水池，池里一块石碑；中部一大片铁丝网地板
围着黑色深坑，有通风口、舱门和宝箱；四周砖墙上二十四根蓝色光柱；底部两角蓝色水池。

| Object | DefinitionKey | Footprint (rows × cols) |
|---|---|---|
| 高光柱 ×16 | `light_pillar_1` | 148 over 156, 152 over 157 |
| 矮光柱 ×8 | `light_pillar_2` | 158 |
| 石碑 | `stone_stele_1` | cap 104 over tablet 109 in the pool |

Resolved -- the map is 31 × 64 tiles, all ordinary tiles; `obstacles_29.json` from
`prop_obstacles.py`. Reflections erased: 112 under the stele, the lone ellipses
162. Fills: 145 (floor) under the light pillars, 146 (water) under the stele's cap
and body (forced for 109 -- the pool's front edge has no plain middle tile), 258
under its reflection.

**The dragon stays painted** (272..382, `Gap`): it is the chapter's boss, drawn
over the top of the map as tiles, and nothing in the prompt asked for a model.
The brick walls (0..37, 176..207, `Gap`), the stairs (136..144), the mesh and
its pits (230 / 231), the vents and hatches (216 / 217) and the chests (224 /
226) stay painted; same `shapes_to_vox.py` flags as chapter 27.

## Chapter 30

最后的祭坛：黑色的虚空里一座灰色的石台，蓝色的水道绕着中央的石阶和一方蓝色的祭坛
（画在瓦片上，事件用），褐色砖墙，十一根蓝色光柱；石台的边缘是撕裂的铁丝网。

| Object | DefinitionKey | Footprint (rows × cols) |
|---|---|---|
| 高光柱 ×6 | `light_pillar_1` | 152 over 157 |
| 矮光柱 ×5 | `light_pillar_2` | 158 |

Resolved -- the map is 45 × 40 tiles, all ordinary tiles; `obstacles_30.json` from
`prop_obstacles.py`, the lone ellipses 162 erased, fills 145. The altar (0..10,
`Gap`), the stairs (136..144), the walls, the water channels (163 / 164) and the
torn mesh edges (272..287) stay painted. Water here is `(40,68,136)`:
`shapes_to_vox.py --flat --lower 000000=3 --lower 284488=2 --lower-min-area 45`.
